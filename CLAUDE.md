...
- **`constants.js`** — `WHATSAPP_NUMBER`, `INSTAGRAM_URL`, `WHATSAPP_URL`. **Replace `91XXXXXXXXXX` with the real WhatsApp number before launch.**
...
- **`images.js`** — single source of truth for all Unsplash image URLs. The `u()` helper builds parameterized URLs. To swap real photos for Unsplash placeholders, replace the relevant URL here.
...
| `blush` | `#E8D5C4` | Subtle accents |
| `gold` | `#C4A882` | Primary accent, CTAs |
| `brown-deep` | `#5C4A36` | Headings, primary buttons |
| `brown-mid` | `#8B7355` | Body text |
| `brown-light` | `#A89070` | Captions, secondary text |
...
- **`Button.jsx`** — exports a default `Button` component and a `buttonClasses(variant, className)` utility. Use `buttonClasses` when you need button styling on a non-`<button>` element (e.g. a `<Link>`). Variants: `solid`, `outline`, `gold`.
- **`ProductCard.jsx`** — uses `buttonClasses` for its "Enquire Now" Link. All enquiries route to `/contact`.
- **`TestimonialCard.jsx`** — layout-agnostic; pass layout classes (min-width, flex-shrink) via the `className` prop.
- **`CategoryCard.jsx`** — links to `/shop?category={id}` using the slug from `categories.js`.
...
- The Navbar uses CSS Grid (`grid-cols-[1fr_auto_1fr]`) for logo centering — do not revert to absolute positioning.
